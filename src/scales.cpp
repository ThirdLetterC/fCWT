...
fcwt::Scales::Scales(const ScaleType st, const int fs, const float f0,
                     const float f1, const int fn)
    : fs(fs), nscales(fn) {
  scales.resize(fn);
  switch (st) {
...
bool fcwt::Scales::check_nyquist_satisfied(const float f,
                                           const int fs) noexcept {
  if (f > static_cast<float>(fs) / 2) {
    [[unlikely]] std::cerr
...
void fcwt::Scales::calculate_logscale_array(const float base, const int fs,
                                            const float f0, const float f1,
                                            const int fn) noexcept {

  // If a signal has fs=100hz and you want to measure [0.1-50]Hz, you need
  // scales 2 to 1000;
  const float nf0 = f0;
  const float nf1 = f1;
  ...
  const float power[2] = {std::log(s0) / std::log(base),
                          std::log(s1) / std::log(base)};
  const float dpower = power[1] - power[0];

  for (int i = 0; i < fn; i++) {
    const float log_power = power[0] + (dpower / static_cast<float>(fn - 1)) *
                                           static_cast<float>(i);
    scales[i] = std::pow(base, log_power);
  }
}

void fcwt::Scales::calculate_linfreq_array(const int fs, const float f0,
                                           const float f1,
                                           const int fn) noexcept {
  ...
  for (int i = 0; i < fn; i++) {
    scales[fn - i - 1] =
        ...
        (nf0 + df / static_cast<float>(fn) * static_cast<float>(i));
  }
}

void fcwt::Scales::calculate_linscale_array(const int fs, const float f0,
                                            const float f1,
                                            const int fn) noexcept {
  // If a signal has fs=100hz and you want to measure [0.1-50]Hz, you need
  // scales 2 to 1000;
