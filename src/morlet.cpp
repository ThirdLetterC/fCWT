...
void fcwt::Morlet::generate(const int size) noexcept {
  ...
  for (int w = 0; w < width; w++) {
    float tmp1 =
        (2.0f * (static_cast<float>(w) * toradians) * fb - 2.0f * PI * fb);
    tmp1 = -(tmp1 * tmp1) / 2;
    mother[w] = norm * std::exp(tmp1);
  }
}

void fcwt::Morlet::generate(std::vector<std::complex<float>> &pwav,
...
void fcwt::Morlet::getWavelet(const float scale,
                              std::vector<std::complex<float>> &pwav,
                              const int pn) noexcept {
